...
export class DemoBridge {
  ...
  item() { return { key: 'DEMO0001', title: 'Evidence before answers — demonstration', type: 'journalArticle', date: 'Demo', authors: 'Synthetic fixture', attachments: 'PDF', hasPdf: true, tags: ['demo'] }; }
  async search() { return { items: [this.item()] }; }
  async library() { return { collections: [{ key: 'DEMOCLL1', name: 'Demonstration collection', depth: 0, parentKey: null }], items: [this.item()], scope: 'recent' }; }
  async collection(key) { if (key !== 'DEMOCLL1') throw new Error('Demo mode only contains one collection.'); return { items: [this.item()], total: 1, offset: 0, nextOffset: null }; }
...
export class DemoProvider {
  async complete(messages, tools, signal) {
    ...
    if (tools?.some(tool => tool.function?.name === 'read_page') && !messages.some(m => m.role === 'tool')) return { tokens: 80, message: { role: 'assistant', content: null, tool_calls: [{ id: 'demo-read', type: 'function', function: { name: 'read_page', arguments: '{"page":2}' } }] } };
